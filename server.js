...
// --- START: bypass-injection guard (paste langsung setelah `const code = await fs.readFile(...);`) ---
/**
 * Jika file terlihat sudah terenkripsi/packed, jangan panggil obfuscator lagi
 * (karena akan gagal/timeout). Sebagai gantinya, kita inject bypass stub
 * di depan file (prepend) lalu kirim hasilnya.
 *
 * Behavior:
 *  - jika includeBypass atau includeAntiBypass true -> tambahkan stub
 *  - jika tidak -> langsung kirim file asli (tanpa re-obfuscate)
 */
const looksEncrypted = /eval\(|eval\(function\s*\(|function\s*\(p,a,c,k,e,d\)|atob\(|Buffer\.from\(|String\.fromCharCode\(|_0x[a-f0-9]{4,}/i.test(code);

if (looksEncrypted) {
  console.log("[info] Detected packed/encrypted input — will NOT re-obfuscate. Injecting bypass stub if requested.");

  // create a bypass stub based on flags sent by frontend
  function createBypassStub({ includeBypass, includeAntiBypass, password }) {
    // Minimal, generic stub that runs BEFORE the encrypted payload.
    // Customize this string to inject whatever bypass logic you need.
    // Keep it short to avoid changing the encrypted payload.
    const lines = [];

    lines.push("(function(){");
    lines.push("  try {");
    lines.push("    // Seren bypass stub (injected) — runs before encrypted payload");
    lines.push("    globalThis.__SEREN_BYPASS = true;");

    if (includeBypass) {
      // placeholder: user-provided bypass hooks (safe minimal defaults)
      lines.push("    // includeBypass: register a noop hook so runtime bypass checks succeed");
      lines.push("    if (typeof globalThis.__seren !== 'object') globalThis.__seren = {};");
      lines.push("    globalThis.__seren.bypass = true;");
    }

    if (includeAntiBypass) {
      lines.push("    // includeAntiBypass: attempt to blunt naive anti-debug checks");
      // Best-effort no-op for Function.prototype.toString (some obfuscators check it)
      lines.push("    try {");
      lines.push("      const origToString = Function.prototype.toString;");
      lines.push("      Object.defineProperty(Function.prototype, 'toString', {");
      lines.push("        value: function() { return origToString.call(this); },");
      lines.push("        configurable: true, writable: true");
      lines.push("      });");
      lines.push("    } catch(e) { /* ignore */ }");
    }

    if (password) {
      // optional: expose password in a known place (if your encrypted payload will read it)
      // WARNING: embedding passwords in plaintext may be insecure; only do if intended.
      lines.push(`    try { globalThis.__SEREN_PASSWORD = ${JSON.stringify(password)}; } catch(e) {}`);
    }

    lines.push("  } catch(e) { /* stub error ignored */ }");
    lines.push("})();");
    lines.push(""); // blank line before payload

    return lines.join("\n");
  }

  const injectStub = createBypassStub({
    includeBypass: includeBypass,
    includeAntiBypass: includeAntiBypass,
    password: password
  });

  // Build output: stub + original encrypted code (no further obfuscation)
  const injected = injectStub + "\n" + code;

  // write temp output and send it
  ...
  console.log("[info] Sent injected file (stub + original)");
